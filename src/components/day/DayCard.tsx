  ...
  const content = (
    <Card className={`flex flex-col gap-3 ${isClickable ? 'cursor-pointer hover:shadow-md transition-shadow' : 'opacity-60'}`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Dia {day.day_number}</p>
          <h2 className="mt-0.5 text-lg font-semibold text-gray-900">{day.title}</h2>
        </div>
